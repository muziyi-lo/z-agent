...
test "流式集成: reasoning→content 阶段切换时输出分隔线"
test "流式集成: 连续 reasoning block 只输出一次 header（合并相邻推理块）"
```
...
- 连续推理块合并：同一推理阶段收到多个 `delta.reasoning_content` 时，只在首次进入推理时输出 header，末次退出时输出 footer，避免重复 header/footer 闪烁
