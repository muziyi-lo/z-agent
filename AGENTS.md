...
| **Agent mode** | `--agent <file>` or `--agent-prompt <inline>` flags set `agent_mode=true`. `--readonly` denies write operations. `--model <provider/model>` overrides default model. `--trust` skips all permission checks. When active, assistant content is wrapped as `[ZAGENT_RESULT]...content...[ZAGENT_END]`. Optional `--result-marker=<name>` adds scoped markers: `[ZAGENT_RESULT:<name>]...[ZAGENT_END:<name>]`. |
